...
@onready var credits_ui_container:Control = $MarginContainer/VBoxContainer/Credits
...
func _on_credits_button_pressed():
	if button_pressed_audio_stream_player:
		button_pressed_audio_stream_player.play(0.2)
	main_ui_container.visible = false
	credits_ui_container.visible = true
	menu_changed.emit('credits')
...
func _on_credits_back_button_pressed():
	if button_pressed_audio_stream_player:
		button_pressed_audio_stream_player.play(0.2)
	credits_ui_container.visible = false
	main_ui_container.visible = true
	menu_changed.emit('main')
